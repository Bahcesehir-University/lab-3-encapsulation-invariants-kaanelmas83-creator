...
public:
    ...
    BankAccount(const string& owner, double initialBalance) {
        ...
        if(owner.empty()){
              throw invalid_argument ("Yanlış İsim");
        }
        owner_=owner;
        if(initialBalance<0){
            throw invalid_argument ("Yanlış Tutar");
        }
    ...
    void deposit(double amount) {
        // TODO: Implement
        if(amount<= 0){
            throw invalid_argument("paranın pozitif olması gerek");
        }
    ...
    void withdraw(double amount) {
        ...
        if(amount>balance_){
            throw runtime_error("Yetersiz para");
        }
        balance_=balance_-amount;
    }
    ...
    void transfer(BankAccount& other, double amount) {
        ...
        if(amount<= 0){
            throw invalid_argument("Transfer miktarı pozitif olmalı");
    }
    withdraw(amount);
...
private:
    ...
    static void validate(const string& pwd) {
        ...
        if(pwd.length()<8){
            throw invalid_argument ("8 karakterden az olamaz");
        }
        if(!hasDigit(pwd)){
            throw invalid_argument("Şifrenin en az 1 sayısı olması lazım");
        }
    }
...
public:
    ...
    void change(const string& oldPassword, const string& newPassword) {
        ...
        if(oldPassword != password_){
            throw invalid_argument("eşleşmiyor");
        }
